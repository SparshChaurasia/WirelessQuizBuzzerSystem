...
def configure():
    ...
    if s != None:
        ...
        for line in lines:
            if (
                not line.startswith(" ")
                and not line.startswith("mon.")
                and "IEEE 802.11" in line
            ):
                wlan = line.split(" ")[0]
...
def check_interfaces():
    ...
    if not bwlan:
        ...
        print(
            ppp
            + " interface was not found. Make sure you are connected to the internet."
        )
        return False
    else:
        print("done.")
...
def start_router():
    ...
    cli.execute_shell(
        ...
        + " -o "
        + wlan
        + " -j ACCEPT -m state --state RELATED,ESTABLISHED"
    )
    ...
    # start dnsmasq
    s = (
        "dnsmasq --dhcp-authoritative --interface="
        + wlan
        + " --dhcp-range="
        + ipparts
        + ".20,"
...
if __name__ == "__main__":
    ...
    if args.command == "configure":
        ...
        if (
            cli.is_process_running("hostapd") != 0
            and cli.is_process_running("dnsmasq") != 0
        ):
